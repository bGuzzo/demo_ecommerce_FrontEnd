...
class RestManager {
  ...
  Future<String> _makeRequest(
    ...
    while (cont < 5) {
      try {
        ...
        print("errore rest Manager: " + err);
        if (delegate != null && !errorOccurred) {
          delegate.errorNetworkOccurred(Constants.MESSAGE_CONNECTION_ERROR);
          errorOccurred = true;
        }
        await Future.delayed(
            const Duration(seconds: 1), () => null); // not the best solution
        cont++;
      }
    }
  }
  ...
  Future<String> makePostRequest(
      ...
      {TypeHeader type = TypeHeader.json, Map<String, String> params}) async {
    return _makeRequest(serverAddress, servicePath, "post", type,
        body: value, value: params);
  }
